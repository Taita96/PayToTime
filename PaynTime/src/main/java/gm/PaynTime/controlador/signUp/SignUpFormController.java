...
public class SignUpFormController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		LOGGER.info("\nSignUpFormController inicializado\n");
	...
	private void onRegisterAction() {
		...
		if (!ControllerGeneralModel.validateEmail(email)) {
			ControllerGeneralModel.alert(Alert.AlertType.WARNING, "Email inválido",
		...
		if (password.length() < 6) {
			ControllerGeneralModel.alert(Alert.AlertType.WARNING, "Contraseña insegura",
		...
		if (!password.equals(confirmPassword)) {
			ControllerGeneralModel.alert(Alert.AlertType.WARNING, "Contraseñas no coinciden",
					"Verifica la confirmación de contraseña.");
			txtConfirmPassword.requestFocus();
			return;
		}

		// Construir entidad. Ajusta nombre/apellido según tu modelo/constraints.
		...
		try {
			...
			nuevoUsuario.setPassword(confirmPassword);
			usuarioServicio.guardarUsuario(nuevoUsuario);
			
			ControllerGeneralModel.alert(Alert.AlertType.INFORMATION, "Registro Exitoso", "Usuario Registrado");
			onCleanAction();
			LOGGER.info("Usuario registrado: email={}, user={}", email, user);
			
		} catch (UsuarioYaExisteException e) {
			LOGGER.error("Error registrando usuario", e);
			ControllerGeneralModel.alert(Alert.AlertType.ERROR, "Error", "No se pudo registrar el usuario:\n" + e.getMessage());
		}

		// Si tu entidad exige nombre/apellido NOT NULL, coloca valores por defecto o
		// agrega campos al formulario.
		// u.setNombre("N/A");
		// u.setApellido("N/A");

		// Usa tu método seguro de setPassword (que hashea internamente)
		// Si tu entidad tiene setPasswordHash directo, cambia esta línea.
